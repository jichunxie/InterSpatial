#' Cell by Spot Correlation Matrix
#'
#' Computes sparse correlation matrix between the single cells and the spatial spots.
#'
...
#'
#' @return Returns the spot by cell correlation matrix, each column is one cell, and each row is a spatial spot
#' @export
...
create_correlation_matrix <- function(dat_scRNA, dat_visium,p_hvg=2000, num_pca=100,alpha=0.05, nCores=NULL)
{
  ...
  unique_cluster_dat_visium <- as.numeric(unique(cluster_dat_visium))
  unique_cluster_sc <- as.numeric(unique(cluster_sc))
...
results_list <- foreach(
  ...
  dat_visium_ind <- intersect(
    spot_present_ind,
    which(match(as.numeric(cluster_dat_visium), unique_cluster_dat_visium[ind1], nomatch = 0) > 0)
  )
  sc_ind <- which(match(as.numeric(cluster_sc), unique_cluster_sc[ind2], nomatch = 0) > 0)
  ...
  if (length(dat_visium_ind) > 0 && length(sc_ind) > 0) {
    ...
    neg_values <- as.numeric(vec[which(as.numeric(vec) < 0)])
    pos_values <- as.numeric(vec[which(as.numeric(vec) > 0)])
    ...
    vec[which(as.numeric(vec) <= thres)] <- 0
...
#'
#' Computes mapping probabilities of the single cells to the spatial spots from the correlation matrix.
#'
#' @param cor_matrix A sparse or dense matrix of spot by cell correlation.
#'
